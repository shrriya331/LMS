...
public class AdminController {
    ...
    public ResponseEntity<?> approve(@PathVariable @Positive Long id) {
        ...
        return ResponseEntity.ok(java.util.Map.of("message", "User approved", "id", u.getId(), "status", u.getStatus()));
    }

    @PostMapping("/reject/{id}")
    public ResponseEntity<?> reject(@PathVariable @Positive Long id, @RequestParam(required = false) String reason) {
        User u = authService.rejectUser(id, reason);
        return ResponseEntity.ok(java.util.Map.of("message", "User rejected", "id", u.getId(), "status", u.getStatus()));
    }
